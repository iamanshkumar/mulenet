...
async function testCSV(file, label, minFlags, maxFlags) {
    ...
    console.log(`${label}: ${flags} flags in ${ms}ms  ${ms <= 30000 ? '✅' : '❌ SLOW'}`)
}

; (async () => {
    console.log('Running MuleNet integration tests...\n')
    ...
    await testCSV('test6_10k_stress.csv', '10K Stress Test', 10, 2000)
    console.log('\n✅ All tests passed!')
})().catch(console.error)
